 ...
 * - 数据只持设置；可行走网格 + SDF 距离场由设置 + 场景内容（地形高度场、网格
 *   光栅化高度场、静态碰撞体）烘焙（framework/navigation/NavSystem），不进场景
 *   JSON——与地形高度场同约定：改设置即重烘焙，场景重开重烘焙；
 * - 覆盖范围自动取所采样源（settings.sourceIds，地形或网格；空 = 场景第一块
 *   地形）的范围，自身 Transform 不参与烘焙与显示（场景级节点，与雾/天空盒同语义）；
...
export class NavAreaNode extends Node {
  ...
  settings: NavAreaSettings = { ...DEFAULT_NAV_AREA_SETTINGS, sourceIds: [] };
