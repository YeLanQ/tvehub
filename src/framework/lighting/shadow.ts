...
//
// 数据形状可在节点 JSON 与灯光组件设置间共享；applyLightShadow 统一写到
// three 灯光的 shadow 上（渲染侧只认 three 对象，不感知配置来源）。
// ---------------------------------------------------------------------------
...
/** 点光阴影贴图降档：立方体贴图要渲染 6 个面，同分辨率开销是平面阴影的 6 倍 */
...
export function lightShadowSignature(c: LightShadowConfig): string {
  return [c.strength, c.bias, c.normalBias, c.near, c.radius].join("|");
}
