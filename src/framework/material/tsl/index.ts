// ---------------------------------------------------------------------------
// GLSL → TSL 转译器（编辑器侧，供 WebGPU 后端渲染着色器效果）。
// 导出：词法/语法/转译三个层次，承载层（nodeMaterialBackend.ts）依赖
// compileHookNode（Hook 片段 → 端口节点）+ translateProgram + TslFnLib 类型。
// ---------------------------------------------------------------------------
