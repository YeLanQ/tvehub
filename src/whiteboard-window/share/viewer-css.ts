...
// 全屏（body.tv-fs）则相反：满幅放映、不给栏让位（让出来的深色区域在整屏语境下就是黑边）。
//
// 三处刻意例外（不随主题，理由在处）：
...
/* ---------------------------------------------------------------------------
   全屏（纯净放映）：画布满幅铺满整屏，不再给浮动栏让位——窗口态的上下留白是
   给栏让的位置，全屏里就成黑边。两栏默认隐藏；H 临时唤出时覆盖在画面上
   （会话内有效，不写 localStorage），退出全屏自动还原。
   --------------------------------------------------------------------------- */
body.tv-fs .tv-stage { padding: 0; }
body.tv-fs .tv-stage > svg { border-radius: 0; box-shadow: none; }
body.tv-fs .tv-bar { display: none; }
body.tv-fs.tv-fs-chrome .tv-bar { display: flex; }
