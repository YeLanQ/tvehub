...
export const MCP_TOOLS: { name: string; description: string }[] = [
  ...
  { name: "node.component.add", description: "给节点添加组件（脚本组件：script=脚本 .ts 相对路径；内置组件：type=rigidBody/collider/light/audioSource/animationClip；一次撤销）" },
...
export const METHOD_TO_COMMAND: Record<string, string> = {
  ...
  "node.set": "node.set",
  "node.component.add": "node.component.add",
