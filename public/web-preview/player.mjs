...
async function main() {
  ...
  // 材质资产：节点只保存 .mat 引用，先按引用预取解析参数（缺失回退默认）
  const materialParams = await loadMaterialParams(rootJson);
  const { cameras, meshes } = buildSceneTree(rootJson, scene, materialParams);
