  ...
  return {
    ...
    ops: {
      ...
      "op.setFsmParam": (k, node, targets) => {
        ...
        for (const t of targets) {
          try {
            k.logicApi.setParam({ id: t.id }, k.strP(node, "param"), k.numP(node, "value"));
          } catch {
          }
        }
      },
      ...
      "op.fireFsm": (k, node, targets) => {
        ...
        for (const t of targets) {
          try {
            k.logicApi.fire({ id: t.id }, k.strP(node, "event"));
          } catch {
          }
        }
      }
...
function createCoreDataModule() {
  ...
  return {
    id: "core-data",
    data: {
      "var.get": (k, node, portId) => portId === "value" ? k.varGet(node.varId ?? "") : void 0,
      ...
      "math.div": (k, node) => {
        ...
        return bv === 0 ? 0 : numIn(k, node, "a") / bv;
      },
      "math.mod": (k, node) => {
        const bv = numIn(k, node, "b");
        return bv === 0 ? 0 : numIn(k, node, "a") % bv;
...
function createCoreContainersModule() {
  ...
  function fsmSwitch(k, node, seen, evName, viaDstPort) {
    ...
    let target;
    if (viaDstPort === "event") {
      if (!evName || !states.includes(evName)) return;
      target = evName;
    } else {
    ...
    frame(k, node, dt) {
      for (const e of k.graph.edges) {
        ...
        if (nowTrue && prev === false) {
          fsmSwitch(k, node, /* @__PURE__ */ new Set(), k.strP(src, "event"), "event");
        }
      }
