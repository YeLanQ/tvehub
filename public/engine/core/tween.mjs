...
class Tween {
  ...
  /** 开始前延时（秒；多次调用取最后一次） */
  delay(seconds) {
    this.__delay = typeof seconds === "number" && Number.isFinite(seconds) ? Math.max(0, seconds) : 0;
    return this;
  }
  /** 循环次数：1 = 单次（缺省）；n = n 次；-1 = 无限循环 */
  loop(count) {
    this.__loops = typeof count === "number" && Number.isFinite(count) ? count < 0 ? -1 : Math.max(1, Math.round(count)) : 1;
    return this;
  }
  /** 往返：偶数次循环反向插值（终点 → 起点） */
  yoyo(on) {
  ...
  __preamble(dt) {
    ...
    if (!this.__firedStart) {
      ...
      (_b = (_a = this.__driver) == null ? void 0 : _a.prepare) == null ? void 0 : _b.call(_a);
      ...
      for (const key of Object.keys(props)) {
        ...
        if (typeof cur === "number" && typeof p === "number" && Number.isFinite(p)) {
          ...
          const keys = Object.keys(startV).filter(
            (kk) => Number.isFinite(startV[kk]) && endV && typeof endV === "object" && Number.isFinite(endV[kk])
          );
...
function clampHex(v) {
  return typeof v === "number" && Number.isFinite(v) ? Math.max(0, Math.round(v)) & 16777215 : 0;
}
...
function makeTween(driver, duration) {
  const t = new Tween();
  t.__driver = driver;
  driver.duration = typeof duration === "number" && Number.isFinite(duration) ? Math.max(0, duration) : 0;
  t.__begin();
  return t;
}
...
const tween = {
  ...
  set timeScale(v) {
    if (typeof v === "number" && Number.isFinite(v)) {
      timeScale = Math.min(1e3, Math.max(0, v));
    }
  }
};
export {
