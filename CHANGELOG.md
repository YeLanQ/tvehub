...
- **assistant**: 思考过程上屏单行滚动条(reasoning_content/reasoning全链路透传)
- **brain**: repos文本资产作神经图动态层(指纹懒重对齐,外部更新免编译入图)+load_repo直答;助手语言归一化前置层(自然语言LLM结构化为类型/锚点/文件校验再交大脑检索);修时间线toolCallId配对错位;docs/工坊目录常驻提示词…
- **graph**: 上下浮动/持续旋转接入内置Tween缓动曲线——字段options下拉通用渲染，bob走三角往返×曲线(默认正弦)，spin角速度按周期脉冲(留空恒速)，补smoke采样回归
- **graph**: 追击目标新增「最小距离」——与目标距离小于该值时截停（单帧步长钳制不越界），0=不限制，补smoke回归
- **graph**: 导航移动新增「恢复续走」勾选——代理暂停恢复时从当前位置续走当前路径点（navApi.setAgentResumeContinue，默认仍回首路径点重走），补smoke方向对照回归
- **graph**: 状态机命名切换事件——容器「切换事件」(事件>状态)映射，比较卡触发事件名下拉只列事件(作用域.fsm优先/过滤失效映射与状态名)，接线自动同步映射；补检查器spec与运行时smoke回归
- **assistant**: 供应商上下文窗口配置（默认128K，可设1024K=1M）——预算内工具结果与压实窗全量放大，超预算才裁最老历史，大窗口模型不再被固定截断卡容量
- **brain**: 文件内模块索引 file.index/file.search——@大文件只注入摘要+模块目录，模型按需检索行号摘录，不再整包进上下文
- **assistant**: 输入框敲 @ 实时补全工作区文件（↑↓/Enter/Esc 键盘流，替换式插入 @token；点 @ 按钮仍自由搜索，两模式共用浮层与资产清单）
- **brain**: 任务分层准确性/模糊原子任务——参数确定性提取（项目名/实体词/路径）即大脑直执行（含链式编排+段内多命令展开），模糊单元仍助手细化经决策中心
- **assistant**: load_doc 直读内嵌文档全文+load_skill 兜底内嵌表，知识命中过滤词元碎片并改目录式注入（按需拉取不预载）
- **brain**: docs 纳入神经图基图元（22篇手册内嵌为Concept群）+词典动宾组合匹配+图谱参考知识随单元转发，修tween脚本类任务检索落空
- **brain**: 输入先经语义单元化(分段→神经图检索→命令预测)拆单元+轨迹上屏过程容器，助手逐单元小循环、决策仍走决策中心，信号不足回落整任务
- **brain**: 助手工具执行权收归后端决策中心——门控(三区+任务审批会话)→命令模式派发(devtools 内部通道)→观测闭环，project.create 下沉 Rust 直答
- **devtools**: project.open 无编辑器窗口时本地兜底新开编辑器（就绪回执防转发丢失）
- **projects**: projects:changed 跨窗口广播（助手/控制端改项目首页即时刷新）+ 助手内联工具增强；lib.rs Mutex 改中毒恢复式加锁
- **audit**: 质量+安全统一扫描器（密钥/XSS/穿越/CVE/复杂度/重复/覆盖率+分级门禁台账），接入 ci:local、build CD 与 audit:rescan 上线后复扫
...
- **assistant**: 工作区同步：新增workspace_path_exists+pruneMissing清已删项目会话(激活回落通用,父位置不可达保守保留),写操作事件驱动文件树自动刷新(保展开)+聚焦补同步+树刷新钮
- **assistant**: 净化补模仿回喂格式的「[工具 X 调用]」伪标记(只剔标记,旁白正文保留)
- **assistant**: 净化补内联think标签(配对/未闭合/孤儿)与复读工具回喂块(标记行惰性吃到系统注/空行/文尾,400字上限防误吞)
- **assistant**: 挂脚本改走组件通道：新增node.component.add(属性面板组件卡同源,支持内置组件/幂等/单实例约束),工具目录与提示词改向+助手通道kind=script语义改写为组件挂载
- **assistant**: 归一化工作区文件一直0个(提示词强约束提取+存在校验后缀兜底对回规范路径),NluSteps补文件校验命中轨迹
- **assistant**: 过滤空json代码围栏残留(围栏调用整体吸附剔除+空围栏清理+流式尾栅隐藏),正文仅围栏调用时占位文本正常接管
- **assistant**: 运行态迁会话级runs修切面板/切会话丢任务视图,任务锚定工作区,设置面板v-show不卸载聊天,流收尾事件驱动,引用解析与拆解并行
- **assistant**: 弱模型标签方言调用真执行+复读空转守卫，工具调用信息单行收敛箭头展开，@引用列表防压扁
- **nav**: 预览导航烘焙把带碰撞体的采样源地形误当障碍（exclude 表漏用→可行走0代理不动）；贴地高度按地形世界Y换算；player接onLog烘焙/启动不再静默，补smoke回归
- **brain**: file.search 缺省 topK 接上 DEFAULT_TOP_K=2（原 clamp 把缺省 0 变成 1），补回归测试
- **assistant**: node.set 补全字段包形式+transform逐轴合并，scene_open 增 force 干净会话磁盘重装（asset.write 直写后 project.open/scene.open 即刷新，脏会话保留不覆盖）
- **brain**: 节点直执行前自动补 project.open（$prev.path 或工作区路径），修缺打开动作致节点直执行全数报无编辑器+半角逗号名字截断
- **assistant**: 工具回喂后纯文本轮默认自动续跑（完成语义/取消才放行收尾），复读救援+暂停注记，续跑预算4→8，修一步一停要手动继续
- **assistant**: 会话脱离 ui-state 独立存 assistant/conversations/（原子写+启动迁移），修挂载竞态空索引覆盖磁盘致丢会话+消息即时落盘
- **brain**: 启动即落基线快照+退出时维护落盘，低频使用的大脑积累不再重启即丢（brain/ 目录首启可见）
- **assistant**: 过程容器图标换 svg 线条图+行元素锁 flex:none 修内容超高被压扁致滚动失效
- **assistant**: project.open 本地兜底解析启动场景交付（编辑器交接只认非空 rel，修卡布防蒙版 0%）+ 宣言救援词表放宽预算 3 + 提示词禁为可选参数停问
- **assistant**: 后台探查静默化（挂载首扫/重扫不弹无编辑器错误）+ 会话树折叠状态持久化跨重启
- **assistant**: 左栏 projects:changed 即时刷新+会话树可折叠（文件夹图标兼开关），通用会话不渲染@按钮免无工作区报错
- **ai**: SSE content 兼容分段数组形态，防供应商变体整段丢正文成空回复
- **assistant**: 任务执行韧性——内联方言解析+跨轮防重复备忘+确认请求早退+空回复自愈，执行过程容器内滚动跟随与收起
- **assistant**: 任务执行加固——多方言调用解析/并行/宣言与空回复救援/24轮预算，执行过程面板+确认浮动条+停止+复制，devtools统一调用日志与服务门控
- **engine**: 场景原型往返丢子树/克隆污染源场景，词法器丢弃 f 后缀字面量
- **engine**: 阴影正交投影基改用阴影相机姿态，smoke 基线同步
