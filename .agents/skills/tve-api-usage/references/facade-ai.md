...
| `devtoolsCall(method, params?)` → unknown | **进程内执行一条 devtools 方法**（权限门控 → Rust 直答 `project.list/project.create/scene.list/asset.list/asset.read/asset.write/scene.tree/scene.save` → 其余转发编辑器执行器回填；60s 超时）。**助手工作区语义**：`scene.list/asset.list/asset.read/asset.write` 支持显式 `root` 参数指定项目目录（不依赖编辑器会话），缺省回退活跃编辑器会话。**助手 LLM 工具调用不走此门面**——统一经 `brainExecute`（决策中心门控后内部同样走这条通道）；此门面供窗口 UI 直调（如助手文件树） |
