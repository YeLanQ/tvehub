...
8. **调试**：`engine.log/warn/error(...)` → 编辑器控制台（postLog 转发）；
   预览面板 F3 开调试统计。
9. **关联文件先查再改**（防改一个文件弄坏引用它的文件）：场景/预制体以
   「脚本相对路径 + 类名」挂脚本组件——重命名/移动/删除脚本前，先用
   file.search 在 .scene/.prefab 文本里搜脚本路径与类名确认引用方，必须
   改名时在总结中说明哪些节点需要重新挂载；改 @property 字段名会让场景
   已存的覆盖值回退默认值。同理：材质 .mat 引用着色器与属性名，删改
   着色器属性会让材质已调参数丢失；模型/音频/材质资产移动或删除前，
   先确认场景无引用。
